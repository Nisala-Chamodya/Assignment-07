...
export class OrderController {
  ...
  handleOrderID() {
    ...
    let x = +t[1];
    x++;
    $("#order_id").text("OD-" + String(x).padStart(5, "0"));
  }
  ...
  handleValidation() {
    $("#customerCmb :selected").text() === "Choose..."
      ? (alert("Please select the customer details !"),
        $("#customerCmb").focus(),
        $("#customerCmb").css({ borderBottom: "2px solid red" }))
      : $("#itemCodeCmb :selected").text() === "Choose..."
      ? (alert("Please select the item details !"),
        $("#itemCodeCmb").focus(),
        $("#itemCodeCmb").css({ borderBottom: "2px solid red" }))
      : !/\d+$/.test($("#qty").val())
      ? (alert("Qty invalid or empty !"),
        $("#qty").focus(),
        $("#qty").css({ borderBottom: "2px solid red" }))
      : $("#qty").val() > $("#qty_on_hand").text()
  ...
  handleUpdateItem() {
    let index = order_item_arr.findIndex(
      (value) => value._item._itemCode === selectedItemCode
    );
    ...
    order_item_arr[index] = new Order_Item(
      itm,
      $("#qty").val(),
      $("#qty").val() * $("#unit_price").text()
    );
  ...
  handleAddItem() {
    ...
    if (index === -1) {
      ...
      parseInt(order_item_arr[index]._qty) + parseInt($("#qty").val()) >
      parseInt($("#qty_on_hand").text())
    ) {
      alert("Noo much qty left !");
      $("#qty").focus();
      $("#qty").css({ borderBottom: "2px solid red" });
    } else {
      order_item_arr[index]._qty =
        parseInt(order_item_arr[index]._qty) + parseInt($("#qty").val());
      order_item_arr[index]._total =
        parseInt(order_item_arr[index]._qty) *
        parseInt($("#unit_price").text());
    }
  ...
  handleDeleteItem() {
    order_item_arr.splice(
      order_item_arr.findIndex(
        (value) => value._item._itemCode === selectedItemCode
      ),
      1
    );
  ...
  handleIsExists() {
    return order_item_arr.findIndex(
      (value) => value._item._itemCode === $("#itemCodeCmb :selected").text()
    );
  }
  ...
  handleSaveOrder() {
    ...
    saveOrderDB(
      new Order($("#order_id").text(), cus, order_item_arr, $("#date").text())
    );
  ...
  handleLoadTable() {
    ...
    order_item_arr.map((value) => {
      var row =
        "<tr>" +
        "<td>" +
        value._item._itemCode +
        "</td>" +
        "<td>" +
        value._item._description +
        "</td>" +
        "<td>" +
        value._item._qtyOnHand +
        "</td>" +
        "<td>" +
        value._item._unitPrice +
        "</td>" +
        "<td>" +
        value._qty +
        "</td>" +
  ...
  handleTableClickEvent() {
    $("#orderTbl tbody").on("click", "tr", (event) => {
      ...
      for (var i = 0; i < arr.length; i++) {
        if (
          arr[i].value === $(event.target).closest("tr").find("td").eq(0).text()
        ) {
          arr.selectedIndex = i;
        }
      }
      ...
      $("#unit_price").text(
        $(event.target).closest("tr").find("td").eq(3).text()
      );
      $("#qty").val($(event.target).closest("tr").find("td").eq(4).text());

      index = order_item_arr.findIndex(
        (value) => value._item._itemCode === $("#itemCodeCmb :selected").text()
      );
